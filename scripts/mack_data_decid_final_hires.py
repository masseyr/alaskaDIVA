...
if __name__ == '__main__':
    ...
    decid_layer = ee.Image("users/masseyr44/decid/decid_mosaic_2000_prediction_vis_nd_3")
    ...
    ak_interior = ee.FeatureCollection("users/masseyr44/shapefiles/alaska_interior_zones")

    tc2000 = ee.Image("users/masseyr44/decid/ABoVE_median_SR_NDVI_boreal_2000_tc_prediction_MM").rename('tree_cover')
    epa_l3 = ee.FeatureCollection("users/masseyr44/shapefiles/alaska_EPA_L3")
    elevation = ee.Image('USGS/GMTED2010').rename('elevation')
    slope = ee.Terrain.slope(elevation)
    aspect = ee.Terrain.aspect(elevation)

    ak_roads_buffer = ee.Image('users/masseyr44/decid/alaska_highways_trails_2km_buffer')

    all_bands = ['id', 'EPA_DESIG', 'FIREID', 'FireYear', 'FireName',
    ...
    buffer_mask = ak_roads_buffer.unmask(0)

    nlcd_mask = nlcd_mask.updateMask(nlcd_mask.eq(0)).unmask(1)
    nlcd_mask = nlcd_mask.updateMask(nlcd_mask.eq(1))

    elev_mask = elevation.lt(elev_lim)

    decid_layer = decid_layer.multiply(nlcd_mask).multiply(elev_mask).multiply(buffer_mask).\
        rename('decid_frac').clip(bounds)
